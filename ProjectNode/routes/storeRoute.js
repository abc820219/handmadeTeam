...
router.get('/:storeId?', (req, res) => {
    ...
    if(req.params.storeId){
        sql = 'SELECT * FROM `store` WHERE `store`.area_sid = '+req.params.storeId;
    }else {
...
router.post('/', (req, res) => {
    ...
    let sql;
    let locateSql = condition.locate_sid?` AND area_sid = ${condition.locate_sid}`:'';
    let accompanyPartnerSql = condition.accompanyPartner?` AND store_partner = 1`:'';
    let accompanyChildSql = condition.accompanyChild?` AND store_child = 1`:'';
    sql = "SELECT * FROM `store` WHERE 1"+ locateSql+accompanyPartnerSql+accompanyChildSql;
