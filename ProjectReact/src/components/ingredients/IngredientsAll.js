...
import { addIngre, cancelIngre } from "../../components/cart/CartAction";
...
const IngredientsAll = props => {
  ...
  const backAllData = async () => {
    const backAllDataLoad = await fetch(
      "http://localhost:5000/handmade/ingredients"
    );
            ...
            >
              {bakeAllDataShow.map(bakeAll => (
                <div className="recipeSliderLeft" id="recipeSliderLoadClick">
                  <div
                    className="recipeSliderLeftBakeName"
                    onClick={() => {
                      selectBakeItem(bakeAll.bake_sid);
                    }}
                  >
                    <p className="recipeSliderLeftBakeNameText">
                      {bakeAll.bake_name}
                    </p>
                  </div>
            ...
            >
              {bakeAllDataShow.map(bakeAll => (
                <div className="recipeSliderLeft">
                  <img
                    src={`/image/ingredientsMin/${bakeAll.bake_image}`}
                    onClick={() => {
                      selectBakeItem(bakeAll.bake_sid);
                    }}
                  />
          ...
          <div className="recipeChooseMain">
            <div className="buttonRightRecipeLeft">
              <div>
                ...
                <a
                  className="clickToIngredientsBuyCard"
                  href="#clickToIngredientsBuyCard"
                >
                  <p>選購食材</p>
                </a>
              </div>
              ...
              />
              <div
                className="ingredientsBuyCardGroup"
                id="clickToIngredientsBuyCard"
              >
                  ...
                  {ingredientData.map(ingredient => (
                    <li className="ingredientsBuyCardLi">
                      <img
                        ...
                        src={`/image/ingredients/${ingredient.ingredients_image}`}
                      />
                      <div className="ingredientsCardGroup">
                        <p className="ingredientsName">
                          {ingredient.ingredients_name}
                        </p>
                        <p className="ingredientsDetail">
                          {ingredient.ingredients_detial}
                        </p>
                        <div className="ingredientsDown">
                          <div className="ingredientsMain">
                            <p className="ingredientsPlace">
                              產地：{ingredient.ingredients_place}
                            </p>
                            <p className="ingredientsSize">
                              尺寸：{ingredient.ingredients_size}
                            </p>
                            <p className="ingredientsPrice">
                              價格：{ingredient.ingredients_price}元
                            </p>
                          </div>
                        </div>
                        <a
                          className="ingredientsCartButton"
                          style={
                            ingreCart.length !== 0 &&
                            ingreSelectLoop(
                              ingredient.ingredients_name,
                        ...
                        >
                          ...
                          <p>
                            {ingreCart.length !== 0 &&
                            ingreSelectLoop(ingredient, ingreCart)
                              ? "取出購物車"
                              : "放入購物車"}
                          </p>
                        </a>
